$(function (w) {
    ...
    $('.sub').click(function () {
        orderNumber--;
        if (orderNumber <= 0) return layer.msg('è´­ä¹°æ•°é‡ä¸èƒ½ä½ŽäºŽ 1 ä»¶');
        numDoc.val(orderNumber);
    })
    $('.add').click(function () {
        orderNumber++;
        if (orderNumber > stock) return layer.msg('è´­ä¹°æ•°é‡ä¸èƒ½å¤šä½™åº“å­˜æ•°é‡');
        numDoc.val(orderNumber + '');
    })
    numDoc.change(function () {
        ...
        if (val <= 0) {
            $(this).val(1);
            orderNumber = 1;
            return layer.msg('è´­ä¹°æ•°é‡ä¸èƒ½ä½ŽäºŽ 1 ä»¶');
        }
        if (val > stock) {
            $(this).val(stock);
            orderNumber = stock;
            return layer.msg('è´­ä¹°æ•°é‡ä¸èƒ½å¤šä½™åº“å­˜æ•°é‡');
        }
        orderNumber = val;
    })
    ...
    w.showQrcode = function (imgUrl) {
        ...
        layer.open({
            type    : 1,
            title   : qrSvg + 'æ‰«æäºŒç»´ç  æ‰‹æœºä¸‹å•',
            closeBtn: 2,
            area    : msgArea,
            skin    : 'home-tips',
            anim    : 5,
